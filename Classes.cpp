...
void DELETE(vector<Media*>* allMedia) {
  ...
  if (strcmp(input2, "TITLE") == 0) {
    ...
    for (cycle = allMedia -> begin(); cycle != allMedia -> end(); ++cycle) {
      ...
      //Asks user if they wish to delete, then deletes it if they wish for it to be deleted, Exits after one deletes, or there are no more search results to be deleted
    ...
    for (cycle = allMedia -> begin(); cycle != allMedia -> end(); ++cycle) {
      ...
      //Asks user if they want to delete, then deletes it if they want it to be deleted. Exited after one deletes, of there are no more search results to be deleted
      cout << "Do you wish to delete this Media? Only yes will delete, everything else is taken as no \n";
      cin.get(deletE, 50);
      cin.clear();
      cin.ignore(9999, '\n');
